...
case class AddIdentificationYesNoPage(activeIndex: Index) extends QuestionPage[Boolean] {
  ...
  override def route(userAnswers: UserAnswers, mode: Mode): Option[Call] =
    Some(routes.AddIdentificationYesNoController.onPageLoad(userAnswers.lrn, mode, activeIndex))

  override def cleanup(value: Option[Boolean], userAnswers: UserAnswers): Try[UserAnswers] =
    value match {
      case Some(false) => userAnswers.remove(IdentificationPage(activeIndex))
