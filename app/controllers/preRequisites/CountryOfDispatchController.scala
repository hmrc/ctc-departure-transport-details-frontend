...
import views.html.preRequisites.CountryOfDispatchView
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode): Action[AnyContent] = actions.requireData(lrn).async {
    implicit request =>
      service.getCountries().map {
        countryList =>
          val form = formProvider("preRequisites.countryOfDispatch", countryList)
          ...
          form
            ...
            .fold(
              ...
              value => {
                ...
                CountryOfDispatchPage.writeToUserAnswers(value).updateTask().writeToSession(sessionRepository).navigateWith(navigator)
              }
            )
      }
  }
