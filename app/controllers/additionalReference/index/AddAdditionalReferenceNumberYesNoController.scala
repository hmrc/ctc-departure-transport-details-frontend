...
class AddAdditionalReferenceNumberYesNoController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, additionalReferenceIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(AddAdditionalReferenceNumberYesNoPage(additionalReferenceIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            AddAdditionalReferenceNumberYesNoPage(additionalReferenceIndex).writeToUserAnswers(value).updateTask().writeToSession().navigate()
          }
        )
  }
}
