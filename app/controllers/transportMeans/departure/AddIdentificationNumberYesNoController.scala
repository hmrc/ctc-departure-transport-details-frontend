...
class AddIdentificationNumberYesNoController @Inject() (
  ...
  def onPageLoad(lrn: LocalReferenceNumber, mode: Mode, departureIndex: Index): Action[AnyContent] = actions.requireData(lrn) {
    implicit request =>
      val preparedForm = request.userAnswers.get(AddIdentificationNumberYesNoPage(departureIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            AddIdentificationNumberYesNoPage(departureIndex).writeToUserAnswers(value).updateTask().writeToSession(sessionRepository).navigateWith(navigator)
          }
        )
  }
}
