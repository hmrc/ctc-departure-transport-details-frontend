  ...
  "AddAnotherSeal Controller" - {
    ...
    "redirect to add seal yes/no page" - {
      "when 0 seals" in {
        ...
        redirectLocation(result).value mustEqual
          controllers.equipment.index.routes.AddSealYesNoController.onPageLoad(lrn, mode, equipmentIndex).url
      }
    }
    ...
    "must populate the view correctly on a GET when the question has previously been answered" - {
      "when max limit not reached" in {
        ...
        setExistingUserAnswers(emptyUserAnswers.setValue(AddAnotherSealPage(index), true))
    ...
    "when max limit not reached" - {
      "when yes submitted" - {
        "must redirect to seal id number page at next index" in {
          ...
          redirectLocation(result).value mustEqual
            controllers.equipment.index.seals.routes.IdentificationNumberController.onPageLoad(lrn, mode, equipmentIndex, Index(listItems.length)).url
        }
      }
