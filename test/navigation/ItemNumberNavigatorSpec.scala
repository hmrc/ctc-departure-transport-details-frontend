        ...
        "must redirect to add another item number page" in {
          forAll(arbitraryGoodsItemNumberAnswers(emptyUserAnswers, equipmentIndex, itemNumberIndex)) {
            answers =>
              navigator
                .nextPage(answers)
                .mustBe(controllers.equipment.index.routes.AddAnotherGoodsItemNumberController.onPageLoad(lrn, mode, equipmentIndex))
          }
        }
      }
    }
        ...
        "must redirect to transport equipment answers" in {
          forAll(arbitraryEquipmentAnswers(emptyUserAnswers, equipmentIndex)) {
            answers =>
              navigator
                .nextPage(answers)
                .mustBe(controllers.equipment.index.routes.EquipmentAnswersController.onPageLoad(answers.lrn, mode, equipmentIndex))
          }
        }
      }
    }
