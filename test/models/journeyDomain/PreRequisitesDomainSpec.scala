  ...
  "PreRequisitesDomain" - {
    ...
    "when post transition" - {
      ...
      "can be parsed from user answers" - {
        "when using same UCR for all items " in {
          ...
          val result = PreRequisitesDomain.userAnswersReader(mockPhaseConfig).apply(Nil).run(userAnswers)
      ...
      "can not be parsed from user answers" - {
        "when answers are empty" in {
          val result = PreRequisitesDomain.userAnswersReader(mockPhaseConfig).apply(Nil).run(emptyUserAnswers)
          ...
          "when TIR" in {
            ...
            mandatoryPages.map {
              mandatoryPage =>
                ...
                val result = PreRequisitesDomain.userAnswersReader(mockPhaseConfig).apply(Nil).run(updatedAnswers)
    ...
    "when transition" - {
      ...
      "can be parsed from user answers" - {
        ...
        "when using the same country of destination" in {
          val userAnswers = emptyUserAnswers
            .setValue(SameUcrYesNoPage, false)
            .setValue(TransportedToSameCountryYesNoPage, true)
            .setValue(ItemsDestinationCountryPage, itemsDestinationCountry)
